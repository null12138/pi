...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const scopeDir = join(root, "@openeryc");
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@openeryc+pi-coding-agent@0.67.68\\node_modules\\@openeryc\\pi-coding-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@openeryc/pi-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts @openeryc/pi-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@openeryc/pi-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@openeryc/pi-coding-agent")).toBe(
			"Update @openeryc/pi-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "@openeryc/pi-coding-agent"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts @openeryc/pi-coding-agent`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@openeryc/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@openeryc\\pi-coding-agent";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@openeryc/pi-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts @openeryc/pi-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "@openeryc/pi-coding-agent"],
			display: "bun install -g --ignore-scripts @openeryc/pi-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@openeryc/pi-coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@openeryc", "pi-coding-agent");
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@openeryc/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@openeryc/pi-coding-agent")).toContain(

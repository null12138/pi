...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@openeryc/pi-ai`, `@openeryc/pi-agent-core`, `@openeryc/pi-coding-agent`, `@openeryc/pi-tui`, `typebox`.

...
**Source:** [`@openeryc/pi-tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@openeryc/pi-tui";
...
```typescript
import { Container, type Focusable, Input } from "@openeryc/pi-tui";
...
Import from `@openeryc/pi-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@openeryc/pi-tui";
```
...
```typescript
import { matchesKey, Key } from "@openeryc/pi-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@openeryc/pi-tui";
...
```typescript
import { getMarkdownTheme } from "@openeryc/pi-coding-agent";
import { Markdown } from "@openeryc/pi-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@openeryc/pi-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@openeryc/pi-coding-agent";
import { DynamicBorder } from "@openeryc/pi-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@openeryc/pi-tui";
...
```typescript
import { BorderedLoader } from "@openeryc/pi-coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@openeryc/pi-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@openeryc/pi-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@openeryc/pi-tui";
...
import { matchesKey, truncateToWidth } from "@openeryc/pi-tui";

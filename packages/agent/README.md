# @openeryc/pi-agent-core

Stateful agent with tool execution and event streaming. Built on `@openeryc/pi-ai`.

## Installation

```bash
npm install @openeryc/pi-agent-core
```

## Quick Start

```typescript
import { Agent } from "@openeryc/pi-agent-core";
...
```typescript
declare module "@openeryc/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@openeryc/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@openeryc/pi-agent-core";
